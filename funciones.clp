  ...
  (foreach ?m (find-all-instances ((?m Menu)) TRUE)
    (if (and
      ...
      then
        ...
        (printout t "Bebida: " (send (nth$ 1 (send ?m get-tieneBebida)) get-nombre) crlf)
...
(deffunction elegir-menu-multiples-bebida (?min ?max ?e)
  (bind ?resultado FALSE)
  (foreach ?m (find-all-instances ((?m Menu)) TRUE)
    (if (and
          (>= (send ?m get-precio) ?min)
          (< (send ?m get-precio) ?max)
          (eq ?resultado FALSE)
          (eq (length$ (send ?m get-tieneBebida)) 3))
      then
        (printout t crlf)
        (printout t "===== Información del Menú de coste " ?e " =====" crlf)
        (printout t "Primer Plato: " (send (send ?m get-tienePrimerPlato) get-nombre) crlf)
        (printout t "Bebida Acompañante: " (send (nth$ 1 (send ?m get-tieneBebida)) get-nombre) crlf crlf)
        (printout t "Segundo Plato: " (send (send ?m get-tieneSegundoPlato) get-nombre) crlf)
        (printout t "Bebida Acompañante: " (send (nth$ 2 (send ?m get-tieneBebida)) get-nombre) crlf crlf)
        (printout t "Postre: " (send (send ?m get-tienePostre) get-nombre) crlf)
        (printout t "Bebida Acompañante: " (send (nth$ 3 (send ?m get-tieneBebida)) get-nombre) crlf crlf)
        (printout t "Precio total: " (send ?m get-precio) crlf)
        (printout t "===============================" crlf crlf)
        (bind ?resultado TRUE)))
    (if (eq ?resultado FALSE)
